...
* Prep for all files.
	...
	putexcel A1=("varlab") `using', `replace'
	putexcel B1=("file") `using', modify
	putexcel C1=("vallab") `using', modify
	if "`recode'" == "" putexcel D1=("recode") `using', modify
	if "`recode'" == "" putexcel E1=("varname") `using', modify
	if "`recode'" != "" putexcel D1=("varname") `using', modify
		if "`recode'" != "" local theNextCol = 5
	...
	* Prep for next file.
		...
		if "`oldname'" != "" {
			local theOldName : word `file_n' of `oldname'
			}
		else local theOldName "oldname`file_n'"
	
		local theNextColumn : word `theNextCol' of `c(ALPHA)'
	
		putexcel `theNextColumn'1=("`theOldName'") `using', modify
	...
	* Write var
		...
		foreach var of varlist * {
			...
			if "`theName'" != "" & "`theName'" != "_" {
				putexcel `theNextColumn'`theNextRow'=("`theName'") `using', modify
				cap putexcel A`theNextRow'=("`theLabel'") `using', modify
				local ++theNextRow
				}
	...
	* Write recode and note columns.
			...
			if "`recode'" != "" foreach column in `recode' {
				local theNextColumn : word `theNextCol' of `c(ALPHA)'
				putexcel `theNextColumn'1=("`column'") `using', modify
					local ++theNextCol
					}
				
			local theNextColumn : word `theNextCol' of `c(ALPHA)'
			putexcel `theNextColumn'1=("notes") `using', modify
			
	* Write labelling sheet and construction sheet
			
		putexcel 		A1=("name") 	B1=("value") 	C1=("label") 		`using', sheet("vallab", replace) modify
			putexcel 	A2=("yesno") 	B2=("0") 		C2=("No") 			`using', sheet("vallab") modify
			putexcel 	A3=("yesno") 	B3=("1") 		C3=("Yes")			`using', sheet("vallab") modify
			putexcel 	A4=("yesnodk") 	B4=("0") 		C4=("No") 			`using', sheet("vallab") modify
			putexcel 	A5=("yesnodk") 	B5=("1") 		C5=("Yes") 			`using', sheet("vallab") modify
			putexcel 	A6=("yesnodk") 	B6=(".a") 		C6=("Don't Know") 	`using', sheet("vallab") modify
	...
	if !regexm("`theNextFile'","dta$") { // Begin import-and-clean loop for Excel files.
		...
		* Generate index variable if specified.
			...
			if "`index'" != "" {
				...
				if "`append'" != "" & `file_n' == 1 {
					qui sum `index'
					local maxGen = `r(max)' // Read in highest current value from constructed data if [append] is specified.
					}
...
* Append to master
	...
	tostring *, replace force // To avoid loss of data during appends.
	append using `master'
		save `master', replace
