...
cap prog drop regchart
prog def regchart
syntax anything /// syntax depvars | indepvars
	, [controls(string asis)] [scatter]
	...
	* Loop over variables
		...
		local alldepvars = substr(" `anything' ",1,strpos("`anything'","|")-1)
			unab alldepvars : `alldepvars'
		local allindvars = substr(" `anything' ",strpos("`anything'","|")+2,.)
			unab allindvars : `allindvars'
				
		foreach var in `alldepvars' {
			
			local y = 0
			foreach indepvar in `allindvars' {
				local oindepvars = subinstr(" `allindvars' "," `indepvar' "," ",.)
	...
	* Combine graph
		...
		local n : word count `allindvars'
		graph combine `theGraphs' , graphregion(color(white)) r(`n') colfirst
