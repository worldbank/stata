...
qui if "`or'" == "or" { // odds-ratio / logistic regressions: bounded by [1/100 , 100]
	...
	use `allData', clear		
		...
		gen m2 = string(round(theResultr12,.01))
			replace m2 = "0" + m2 if theResultr12 < 1
			rename ///
		...
		tw 	(rcap theResultll theResultul n, horizontal lc(black)) ///
			...
			, 	xscale(log) xlab(0.01 "1:100" 0.1 "1:10" 1 "1:1" 10 "10:1" 100 "100:1", notick) ylab(`labels', angle(0) nogrid notick) ///
		...
		if `"`using'"' != `""' {
			...
			rename theResult* *
			export excel label y N x y0 N0 x0 y1 N1 x1 b se z pvalue ll ul `using' , replace first(var)
			}
